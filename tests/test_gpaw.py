...
class GPAWTest(unittest.TestCase):
    ...
    def test_gpaw_defect_relax_set_custom(self):
        ...
        relax_set = GPAWDefectRelaxSet(
            self.structure, charge_state=-1, gpaw_settings=gpaw_settings, poscar_comment="Test Comment"
        )
        relax_set.write_input(self.output_dir)
    ...
    def test_gpaw_kumagai_correction_mgo(self):
        ...
        if os.path.exists(v_mg_unrelaxed_dir):
            ...
            calculated_v_mg_unrelaxed = float(
                v_mg_unrelaxed_entry.corrections["kumagai_charge_correction"]
            )
        ...
        if os.path.exists(mg_o_unrelaxed_dir):
            ...
            calculated_mg_o_unrelaxed = float(
                mg_o_unrelaxed_entry.corrections["kumagai_charge_correction"]
            )
