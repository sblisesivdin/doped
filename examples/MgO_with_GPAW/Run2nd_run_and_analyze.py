...
for d in os.listdir(base_dir):
    ...
    # Check if it's a directory, it's not the bulk folder, and it has a relax.py script
    if os.path.isdir(full_path) and full_path != bulk_dir:
        if os.path.exists(os.path.join(full_path, "relax.py")):
            defect_folders.append(full_path)
...
# --- Cleanup to prevent GPAW garbage collection errors at exit ---
import os
os._exit(0)
